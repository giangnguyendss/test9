    ...
    for row in df.collect():
        ...
        try:
            ...
            # If the view name is null, load the query results directly into the target table
                ...
                target_table = metric_config_df.filter(F.col("metric_id") == row.metric_template_name).first().target_table
                spark.sql(sql_query).write.format("delta").mode("append").option("mergeSchema", "true").saveAsTable(target_table)
