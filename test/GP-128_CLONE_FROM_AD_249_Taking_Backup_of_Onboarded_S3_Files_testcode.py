...
def migrate_files(df):
    ...
    for row in df.collect():
        ...
        try:
            ...
            audit_log = spark.createDataFrame([(file_name, "MOVE", source_path, target_path, "SUCCESS", None, None)], audit_schema)
            ...
            # Create an audit log entry
            audit_log = spark.createDataFrame([(file_name, "MOVE", source_path, target_path, "FAILED", str(e), None)], audit_schema)
...
# Test the migrate_files function
def test_migrate_files():
    # Create a test DataFrame
    test_df = spark.createDataFrame([("file1.txt", "s3://agilisium-playground-dev/filestore/purgo/patient_raw", "s3://agilisium-playground-dev/filestore/purgo/patient_raw_archive")], ["file_name", "s3_landing_path", "s3_archive_path"])

    # Migrate the files
    migrate_files(test_df)

    # Check if the file was moved
    assert dbutils.fs.ls("s3://agilisium-playground-dev/filestore/purgo/patient_raw_archive/file1.txt") != []

    # Check if an audit log entry was created
    audit_log = spark.read.format("delta").table("purgo_playground.wrk_s3_file_migration_audit")
    assert audit_log.filter(audit_log.file_name == "file1.txt").count() == 1

# Test the get_files_to_migrate function
def test_get_files_to_migrate():
    # Create a test DataFrame
    test_df = spark.createDataFrame([("file1.txt", "s3://agilisium-playground-dev/filestore/purgo/patient_raw", "s3://agilisium-playground-dev/filestore/purgo/patient_raw_archive", "SUCCESS")], ["file_name", "s3_landing_path", "s3_archive_path", "file_status"])

    # Write the test DataFrame to the s3_file_process_log table
    test_df.write.format("delta").mode("append").saveAsTable("purgo_playground.s3_file_process_log")

    # Get the files to migrate
    files_to_migrate = get_files_to_migrate()

    # Check if the file is in the files to migrate
    assert files_to_migrate.filter(files_to_migrate.file_name == "file1.txt").count() == 1

# Run the tests
test_migrate_files()
test_get_files_to_migrate()
